...
import com.mulesoft.nn.report.ClassificationModelEvaluationReport;
...
public class NT3 {
	...
	public static void main(String[] args) {
		NT2.initOften();
		PreparedDataSetArea<NTPair> prepare = Workspace.getDefault().prepare(createDataSource(), true);

		CompleteModel model = prepare.input().dense(ActivationFunction.RELU, 250, 40, 5)
				.outBlock(ActivationFunction.SOFTMAX, LossFunction.CATEGORICAL_CROSS_ENTROPY).toModel();
		prepare.setMaxEpochCount(20);
		Experiment<NTPair> experiment = prepare.experiment("a", model, new LearningScheme(LearningAlgoritm.ADAM, 1000));
	...
	static PreseparatedDataSource<NTPair> createDataSource() {
		mappings = getMappings();
		Random random = new Random(23223);
		ArrayList<NTPair> testPairs = new ArrayList<>();
		ArrayList<NTPair> trainPairs = new ArrayList<>();
		ArrayList<NTPair> allPairs = new ArrayList<>();
		for (TestElement e : mappings) {
			...
			if (random.nextInt(5) == 4) {
				...
				//testPairs.addAll(Arrays.asList(e.pairs));
				e.test = true;
			} else {
				if (!e.pairs.isEmpty()) {
					e.pairs.forEach(v -> {
		...
		return new PreseparatedDataSource<>("tsa2", allPairs, trainPairs, testPairs, NTPair.class);
	}
